...
void eval(char *cmdline) 
{
  char *argv[MAXARGS];
  pid_t pid;
  int bg = parseline(cmdline, argv); //returns 1 if this is a background job
  sigset_t sigset;
  sigemptyset(&sigset);
...
int builtin_cmd(char **argv) 
{
  if(strcmp("quit",argv[0])==0 && argv[1]==NULL){
    for (int i=0; jobs[i].state!=UNDEF; i++){
      if(jobs[i].state==ST){
        printf("There exist jobs that are stopped\n");
        return 1;
      }
    }
    exit(0);
...
void do_bgfg(char **argv) 
{
  ...
  if(strcmp(argv[0],"bg")==0 && getjobpid(jobs, jobid)->state==ST){
    getjobpid(jobs, jobid)->state=BG;
    printf("[%d] (%d) %s", getjobpid(jobs,jobid)->jid, getjobpid(jobs,jobid)->pid, getjobpid(jobs,jobid)->cmdline);
    kill(-jobid, SIGCONT);
  }
  else if(strcmp(argv[0],"fg")==0 && getjobpid(jobs, jobid)->state==ST){
    getjobpid(jobs, jobid)->state=FG;
    kill(-jobid, SIGCONT);
    waitfg(jobid);
  }
  else if(strcmp(argv[0],"fg")==0 && getjobpid(jobs, jobid)->state==BG){
    getjobpid(jobs, jobid)->state=FG;
    waitfg(jobid);
  }
  return;
...
{
    struct job_t *s;
    s=getjobpid(jobs, pid);
    while(s!=NULL && s->state==FG)
        sleep(1);
    return;
}
...
void sigchld_handler(int sig) 
{
    ...
    while((pid=waitpid(-1,&status,WNOHANG|WUNTRACED))>0){ //returns pid of child if OK,0 or -1 on error 
        ...
        if(WIFSTOPPED(status)){
            ...
            s->state = 3; //changing the state of stopped jobs to ST i.e. 3
        }
    }
    return;
}
